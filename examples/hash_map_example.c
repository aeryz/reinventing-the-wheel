...
//This is hash_fun to get hash value of given key, in first example our key is char* so we are casting
//void * type to char *, at the end of function we are taking modular 30 because our bucket size is 30.
//So our key should be between 0-29.
...
//If our key is char*(string) then we have to compare keys inorder to push to map or get from map.
int cmp_fun_str(const void* lhs, const void* rhs){
    return strcmp((char*)lhs, (char*)rhs) == 0;
}

void example_str_to_int_fun(){
    //in this example we will use our hashmap like std::unordered_map<std::string,int>
    //so our key is char*, our value is integer.
    ...
    //To push pairs into our hash map our function is "rtw_hmap_insert", it takes 3 parameter.
    //First one is address of our map, second one is our key and third one is address of our data.
    rtw_hmap_insert(&map, key1, &data1);
    rtw_hmap_insert(&map, key2, &data2);
    rtw_hmap_insert(&map, key3, &data3);


    //Now lets try to get our data by using keys. To do that our function is "rtw_hmap_get", it takes 3 arg.
    //First one is address of our map, second one is our key and third one is address of our out data.
    //If thereis value with given key it returns 1 otherwise it returns 0.
    int out_data1;
    int out_data2;
    int out_data3;
    ...
    //To update key-value pair we just have to use insert function with different value.
    if(rtw_hmap_get(&map, key2, &out_data2))
        printf("map[%s]:%d\n",key2, out_data2);
    else
        printf("key: %s couldn't found!\n", key2);
...
//SO our key will be square object, to compare functions lets compare both height and width of square.
...
int hash_fun_struct(const void* key){
    Square *s = (Square *)key;
    return s->width%30;
}
...
void example_struct_to_int_fun(){
    ...
    rtw_hmap map = rtw_hmap_init(&hash_fun_struct, &comp_fun_struct, sizeof(int));
    ...
    if(rtw_hmap_get(&map, &s1, &out_data1))
        printf("map[(Square)->width:%d, (Square)->height:%d]:%d\n",s1.width, s1.height, out_data1);
    else
        printf("key: '(Square)->width:%d, (Square)->height:%d' couldn't found!\n",s1.width, s1.height);

    if(rtw_hmap_get(&map, &s2, &out_data2))
        printf("map[(Square)->width:%d, (Square)->height:%d]:%d\n",s2.width, s2.height, out_data2);
    else
        printf("key: '(Square)->width:%d, (Square)->height:%d' couldn't found!\n",s2.width, s2.height);
    ...
    if(rtw_hmap_get(&map, &s4, &out_data4))
        printf("map[(Square)->width:%d, (Square)->height:%d]:%d\n",s4.width, s4.height, out_data4);
    else
        printf("key: '(Square)->width:%d, (Square)->height:%d' couldn't found!\n",s4.width, s4.height);

    //Now lets try to access our key3 and delete it from hmap.
    if(rtw_hmap_get(&map, &s3, &out_data3))
        printf("map[(Square)->width:%d, (Square)->height:%d]:%d\n",s3.width, s3.height, out_data3);
    else
        printf("key: '(Square)->width:%d, (Square)->height:%d' couldn't found!\n",s3.width, s3.height);

    //Our key3 is in hmap, so now lets try to delete it.
    if(rtw_hmap_del(&map, &s3))
        printf("key: (Square)->width:%d, (Square)->height:%d deleted from hmap.\n",s3.width, s3.height);
    else
        printf("key: (Square)->width:%d, (Square)->height:%d does not exist on hmap.\n", s3.width, s3.height);
