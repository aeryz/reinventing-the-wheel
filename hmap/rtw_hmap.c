...
rtw_hmap rtw_hmap_init(rtw_hmap_hash_fn hash_func, rtw_hmap_cmp_fn comp_func,
    ...
    map.key_len   = key_len;
    map.keys_and_values = rtw_vec_init(key_len + value_len);
    ...
    while (curr_node != NULL) {
        if (map->comp_func(key, curr_node->key)) {
            if (prev_node == NULL) {
                ...
                void *last_elem = rtw_vec_last(&map->keys_and_values);
        
                memcpy(curr_node->key, last_elem, map->key_len);
                memcpy(curr_node->key+map->key_len, last_elem+map->key_len, map->value_len);
            ...
            void *last_elem;
            rtw_vec_pop_back(&map->keys_and_values, last_elem);
...
int rtw_hmap_get(rtw_hmap *map, void *key, void *out_value) {
    ...
    while (NULL != last) {
        if (map->comp_func(key, last->key)) {
            memcpy(out_value, last->key+map->key_len, map->value_len);
            return 1;
        }
        last = last->next;
    }
...
void rtw_hmap_insert(rtw_hmap *map, void *key, void *data) {
    ...
    rtw_hmap_elem_ *last = map->elements[index];
    if (last == NULL) {        
        unsigned char temp[(map->key_len + map->value_len) / sizeof(char)];
        memcpy(temp, key, map->key_len);
        memcpy(temp + map->key_len, data, map->value_len);
        
        rtw_vec_push_back(&map->keys_and_values, temp);

        rtw_hmap_elem_ *item = (rtw_hmap_elem_ *)malloc(sizeof(rtw_hmap_elem_));
        item->next = NULL;
        item->key = map->keys_and_values.data + (map->keys_and_values.len-1) * (map->key_len + map->value_len);
    ...
    for (;;) {
        if (map->comp_func(key, last->key)) {
            memcpy(last->key+map->key_len, data, map->value_len);
            return;
        }
        if (last->next == NULL)
            break;
...
void rtw_hmap_free(rtw_hmap *map) {
    ...
    rtw_vec_clear(&map->keys_and_values, 1);
    rtw_vec_shrink(&map->keys_and_values);
