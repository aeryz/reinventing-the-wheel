...
typedef int (*rtw_hmap_eq_fn)(const void *, const void *);
...
typedef struct {
    rtw_hmap_hash_fn hash_fn;
    rtw_hmap_eq_fn eq_fn;
 ...
 *
 * @param cmp_fn Is function pointer to add our datas to hash map, comparasion
 * function. Help of this comp_fn we will know if incoming key exists in map or
 ...
 * @return if deletion operation is success it returns 1, otherwise it returns
 0.
 */
int rtw_hmap_del(rtw_hmap *self, const void *key);
 ...
 *
 * @return 1 if element found and out_value has been written with that value, 0
 * otherwise.
 */
